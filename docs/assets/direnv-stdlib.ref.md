...
direnv="/Users/joe/.nix-profile/bin/direnv"
...
for var in "$@"; do
    if [[ "$environment" != _"$var="_ || -z ${!var:-} ]]; then
...
discard=false # shellcheck disable=SC2068
...
env_config=$env_spec
  elif [[ "${env_spec%%/\*}" == "." ]]; then # "./foo" relative prefix
realpath.absolute "$env_spec"
    env_loc="$REPLY"
elif [[! "$env_spec" == "${env_spec#/}"]]; then # "/foo" absolute prefix
env_loc="$env_spec"
  elif [[ -n "$env_spec" ]]; then # "name" specified
env_name="$env_spec"
else # Need at least one
...
if [[-z "$env_loc"]]; then # Update location if already created
...
rvm() {
unset rvm
if [[-n ${rvm_scripts_path:-}]]; then # shellcheck disable=SC1090,SC1091
source "${rvm_scripts_path}/rvm"
  elif [[ -n ${rvm_path:-} ]]; then
    # shellcheck disable=SC1090,SC1091
    source "${rvm_path}/scripts/rvm"
else # shellcheck disable=SC1090,SC1091
...
if [[-e "$nodenv_version"]]; then # Put the selected node version in the PATH
NODE_VERSIONS="${node_versions_dir}" NODE_VERSION_PREFIX="" use_node "${node_version}" # Add $PWD/node_modules/.bin to the PATH
    layout_node
  else
...
if [[-f $direnv_config_dir/direnvrc]]; then # shellcheck disable=SC1090,SC1091
